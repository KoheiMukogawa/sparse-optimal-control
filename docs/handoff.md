...
1. **L1スパースMPCノード実装**（rover/にl1_mpc_follower.pyとして追加）
   - RPi上でcvxpyの求解時間を計測してから本実装
   - 比較ベースライン（Kanayama）の実機データはすでにある（bag7本）
2. **rosbag解析スクリプト**: RMSE・入力ノルム・ゼロ入力率を算出する
   パイプライン（uvでrosbagsパッケージを使えばラップトップで可）
3. 補正後トレッド定数での直進再検証（影響軽微の見込み、ついでで可）
