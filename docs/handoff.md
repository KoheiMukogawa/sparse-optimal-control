...
1. **回転キャリブレーション**: その場でN回転コマンド→実回転角を計測し、
   旋回スリップ／トレッド定数の補正係数を同定（実測80°/指令90°が再現するか）
2. **L1スパースMPCノード実装**（rover/にl1_mpc_follower.pyとして追加）
   - RPi上でcvxpyの求解時間を計測してから本実装
   - 比較ベースライン（Kanayama）の実機データはすでにある
3. **rosbag解析スクリプト**: 4本のbagからRMSE・入力ノルム・ゼロ入力率を
   算出するパイプライン（uvでrosbagsパッケージを使えばラップトップで可）
