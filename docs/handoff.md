...
- テスト 71→**81本 all green**（`tests/test_sweep_grid.py` 6本・`test_sweep_delay.py` 4本）
- 次: 実機は R0 待ち。sim 側は **S6（bang-off-bang図の再生成）→ S9（既存bag12本から
  論文図）→ S12（real/sim散布図）**。第2章の執筆は実機・sim いずれにも依存せず着手可能。
