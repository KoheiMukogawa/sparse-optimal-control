# Handoff アーカイブ（2026-06-12〜2026-07-16 セッション13）

最新セッションは docs/handoff.md。ここは過去セッションの全文退避先
（セッション開始時の読み込みを軽くするため 2026-07-17 に分割）。
...
- 最終レビュー（全体・fable）: Important 1件を修正して **Ready with notes**。
  - 修正: キャリブ動画と本番動画の**解像度・向き不一致を無警告受理**していた
    （K誤適用でもタグは全数検出＝例外なしで位置誤差9〜18cmが黙って出る、合成で実測）
    → image_size を設定必須にし、run_video が全フレームで検証・不一致は CalibError。
    calibrate_camera も実フレームサイズを出力（従来は 2*cx の概算）。テスト37本全pass。
  - 実写時の残余注意（レビューNotesの要点）: ①キャリブと本番は同解像度・同じ持ち方
    （手順書に反映済み） ②iPhone可変fps時の時刻は等間隔近似（終点評価には影響軽微。
    時系列突き合わせ時は CAP_PROP_POS_MSEC へ） ③フレーム数が取れない動画は
    calibrate_camera が全フレーム保持（1080p 30sで≈1.8GB、WSL2注意） ④同一IDの
    二重写り（予備タグ・モニタ映り込み）は後勝ちで黙って上書き＝画角に入れない
    ⑤yaw_offset の符号規約は手順書の一発確認でチェック。
...
- **外部計測（真値）＝取得済み（当日夜に床テープ実測を転記）**: MPC 3条件×3本、
  ゴール原点座標(x=コーナー外側+, y=行き過ぎ+)で `external.md` に記録
  （写真 `external_photo.jpeg`、kanayama は未計測）。
  - 終点誤差: **l2 12.7±1.0 < l1_ms2 21.2±11.6 < l1 29.9±9.7 cm** ＝ odomと同序列。
    最終向きは l1_ms2 が最良（1〜10°）、l2 は一貫して16〜22°傾き残り。
  - l1 チャタは終点にも波及（41cm大外れ）。l1_ms2 は rep2/3 が14〜15cm・θ≈1〜4°と
    優秀だが rep1 が34.5cm外れ＝n=3では l2 に及ばず。
  - 全9走行がコーナー外側へ流れる（x>0）＝odom不可視の滑り系統誤差。
    真値絶対値はodom RMSEの5〜10倍 → カメラ真値の必要性を再確認。
  - 注意: 6/13 l2単発（2cm・0°）と今回（13cm・19°）が不一致。単発値は信頼しない。
...
- **実機バッチの事前準備（2026-07-15 追記）**:
  ...
  - RPi側準備: **完了（同日追記）**。RPiのIPがDHCPで .31→**.32** に変動していた
    （.31は別デバイスが応答・SSH拒否。ポート22走査で発見、ホスト鍵一致で本人確認）。
    ①ROS環境チェック合格: setup.bash は想定パス、ROS_SETUP前置で ros2/rclpy とも通る
    （修正の実機検証済み）②mpc_follower.py / mpc_core.py が古かったので scp 配置、md5全5ファイル一致
    ③SSH_HOSTS に .32 を先頭追加、CLAUDE.md の接続先も更新。恒久対策はルーターのMAC固定割当（未実施）。
    残りはバッテリー充電のみ。次回はそのまま実機バッチへ（nav_base起動→スモーク1本→12本）。
