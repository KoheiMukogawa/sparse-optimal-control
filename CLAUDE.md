...
- カメラ真値の実走行検証: **合格（2026-07-16）** → 1m L字（C270・タグはコースを
  囲む配置）で手実測と2.5cm/1.6°一致、odom不可視の滑り+7.9cmを検出。
  コース実寸99/100cm。教訓「床タグはコースを囲む・片側偏在はゴールが外挿で10cm超誤差」
- カメラ真値 Phase 2+3（全自動化）: **実装完了（2026-07-16）・実機E2E未実施** →
  `rover/truth_live.py`（ライブ検出）, `rover/udp_twist_bridge.py`（RPi・watchdog）,
  `rover/homing.py`（原点復帰±3cm/±5°）, `run_batch --auto`（一括許可ループ・
  q+Enter停止・truth列追加）。計画: plans/2026-07-16-camera-truth-phase23.md
- 次: usbipd で C270 を WSL に attach → Phase 2+3 実機E2E
  （手順: docs/作業記録/全自動バッチ運用手順.md。ブリッジ疎通→homing単体→
  短autoループ→フルバッチ）→ 外乱条件バッチを全自動で反復 →
  中間発表ストーリー整理（問題→原因→対策→実機立証）
- 方針変更（2026-07-16）: **広角カメラは買わない**。自動計測は C270＋1m L字
  （対角置き・検証済み2.5cm精度）で完結させる。1.5mが必要になったら
  C270の斜め見下ろし設置を試す（設営写真1枚で可否判定できる）。
  方眼紙の手計測は静置照合の答え合わせ用に限定
